...
class _ViewerPageState extends State<ViewerPage> {
  ...
  int _page = 1;
  // 0 until known. Previously nothing tracked which page was actually on
  // screen — editing always silently targeted page 1 no matter how far you
  // scrolled the continuous viewer, so taps on any other page either
  // selected the wrong thing or found nothing. A real page selector plus
  // PdfViewerController keeps the two in sync.
  int _pageCount = 0;
  ...
  Future<void> _pickAndOpen() async {
    ...
    try {
      ...
      _engine.open(file);
      final count = await MuPdfService.pageCount(file.path);
      setState(() {
        _file = file;
        _selected = null;
        _loading = false;
  ...
  /// Explicitly selects the page to edit (and scrolls the viewer to it),
  /// keeping `_page` — what edit operations actually act on — in sync with
  /// what's on screen instead of it silently staying at 1.
  Future<void> _goToPage(int n) async {
    if (_pageCount > 0 && (n < 1 || n > _pageCount)) return;
    if (n == _page) return;
    setState(() {
      _page = n;
      _selected = null;
    ...
    setState(() => _loading = true);
    await _engine.analyzePage(n);
    final c = _engine.cached(n);
    if (mounted) {
      setState(() {
        _loading = false;
        _analysisSource = c?.source ?? '';
      });
    }
  }
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          _editMode
                              ? 'Editing page $_page of $_pageCount'
