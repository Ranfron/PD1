...
class _OcrPageState extends State<OcrPage> {
  ...
  Future<void> _runOcr() async {
    ...
    final usingPp = wantPp && _ppOcrReady;
    setState(() {
      _processing = true;
      // Only the on-device ONNX pipeline can run long enough on a dense
      // page to need a manual way out; ML Kit alone finishes in under a
      // second either way.
      _showCancelOption = usingPp;
    });
    try {
      String text = '';
      String used = 'ML Kit';
      ...
      if (usingPp) {
        ...
        // Automatic fallback if PP-OCR returns empty (including when it
        // was cancelled or timed out — see PpOcr for the timeout/cancel
        // logic that guarantees this call can't hang forever).
  ...
  /// Asks the native ONNX pipeline to stop as soon as it can. We don't force
  /// `_processing` to false here: the still-running `_runOcr()` call comes
  /// back quickly once native stops, and its existing empty-result fallback
  /// hands off to the fast ML Kit path and finishes normally.
  void _cancelOcr() {
    PpOcr.cancel();
    if (mounted) {
      PdfFilePicker.showResult(
        context,
        'Stopping the on-device model, switching to ML Kit…',
      );
    }
  }
      ...
      body: Stack(
        children: [
          ...
          if (_processing)
            LoadingOverlay(
              message: 'Running OCR…',
              subMessage: _showCancelOption
                  ? 'On-device model — a dense page can take a while.'
                  : null,
              onCancel: _showCancelOption ? _cancelOcr : null,
            ),
        ],
      ),
    );
...
class _PpOcrModelSheetState extends State<_PpOcrModelSheet> {
  ...
  bool _rec = false;
  bool _dict = false;
  // Per-item progress only — deliberately no single shared "busy" flag, so
  // det/rec/dict (and manual "Download All") can all download in parallel
  // instead of every other button being disabled while one is running.
  ...
  Future<void> _downloadDet() async {
    if (_detProgress >= 0) return; // already downloading
    setState(() => _detProgress = 0);
    try {
      ...
      if (mounted) {
        PdfFilePicker.showResult(
          context,
          'DET download failed, will resume next time: $e\n'
  ...
  Future<void> _downloadRec() async {
    if (_recProgress >= 0) return; // already downloading
    setState(() => _recProgress = 0);
    try {
      ...
      if (mounted) {
        PdfFilePicker.showResult(
          context,
          'REC download failed, will resume next time: $e\n'
  ...
  Future<void> _downloadDict() async {
    if (_dictProgress >= 0) return; // already downloading
    setState(() => _dictProgress = 0);
    try {
      ...
      if (mounted) {
        PdfFilePicker.showResult(
          context,
          'Dict download failed, will resume next time: $e',
          isError: true,
        );
      }
    } finally {
      if (mounted) {
        setState(() => _dictProgress = -1);
        await _refresh();
      }
    }
  }
  ...
  Future<void> _downloadAll() async {
    // Kick off det/rec/dict together instead of one after another — each
    // writes to its own file, so there's no reason to serialize them.
    await Future.wait([
      if (_detProgress < 0) _downloadDet(),
      if (_recProgress < 0) _downloadRec(),
      if (_dictProgress < 0) _downloadDict(),
    ]);
  }
        ...
        return ListView(
          ...
          children: [
            ...
            Text(
              ...
              'Downloads resume automatically if interrupted, and can run in parallel.',
