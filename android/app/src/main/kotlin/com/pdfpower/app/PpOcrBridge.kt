...
class PpOcrBridge(private val channel: MethodChannel) :
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            // Deliberately NOT posted to `executor`: if processImage/analyzePage
            // is currently running there, the executor's single worker thread
            // is busy, and anything queued behind it (including this) would
            // just wait until that call finishes — defeating the point of
            // cancelling it. requestCancel() only flips a @Volatile flag, so
            // it's safe to call straight from this thread.
            "cancel" -> {
                try {
